...
export type ClassificaScope = Categoria | 'both';

@Injectable()
export class PdfClassificaService {
  constructor(private classifica: ClassificaService) {}

  async stream(scope: ClassificaScope, res: Response, from?: Date, to?: Date, eventoIds?: number[]) {
    const cats: Categoria[] = scope === 'both' ? ['turismo', 'pista'] : [scope];
    const datasets = await Promise.all(
      cats.map(c => this.classifica.build(c, from, to, eventoIds).then(d => ({ cat: c, ...d })))
    );
    ...
    datasets.forEach((data, idx) => {
      ...
      doc.fontSize(16).font('Helvetica-Bold').text(`Classifica ${data.cat.toUpperCase()} — Porsche Club Roma`, 100, 35);
