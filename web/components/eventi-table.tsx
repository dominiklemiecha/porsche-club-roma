...
function formatPeriodo(e: Evento): string {
  const ini = new Date(e.data_evento).toLocaleDateString('it-IT');
  if (!e.data_fine) return ini;
  const fine = new Date(e.data_fine).toLocaleDateString('it-IT');
  return ini === fine ? ini : `${ini} – ${fine}`;
}

export function EventiTable({ rows, readOnly = false, onEdit, onDelete, onAddPartecipanti }: Props) {
  return (
    <>
          ...
          <div key={e.id} className="rounded-md border border-ink/10 bg-paper p-3 shadow-sm">
            <div className="flex items-start justify-between gap-2">
              <div className="min-w-0">
                ...
                <div className="mt-0.5 text-xs text-ink/60">{formatPeriodo(e)}</div>
              </div>
            ...
            <div className="mt-3 grid grid-cols-2 gap-2">
              ...
              <Button size="sm" variant="outline" disabled={readOnly} onClick={() => onAddPartecipanti(e)}>Partecipanti</Button>
              <Button size="sm" variant="outline" disabled={readOnly} onClick={() => onEdit(e)}>Modifica</Button>
              <Button size="sm" variant="destructive" disabled={readOnly} onClick={() => onDelete(e)}>Elimina</Button>
            </div>
          </div>
        ))}
